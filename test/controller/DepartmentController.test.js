import request from 'supertest';
import app from '../../src/index';
import initTestDatabase from '../config/initTestDatabase';
import Department from '../../src/models/Department';

jest.mock('../../src/middleware/authenticationMiddleware.js', () =>
  jest.fn(() => jest.fn((req, res, next) => next()))
);
jest.mock('../../src/util/documentation/swaggerProps.js', () => jest.fn());
beforeEach(async () => {
  await initTestDatabase();
});

describe('DepartmentController', () => {
  describe('createDepartment', () => {
    it('should return correct information', async () => {
      const response = await request(app)
        .post('/master/department/save')
        .field('department_id', '1')
        .field('department_code', 'HR')
        .field('department_name', 'Human Resources')
        .field('isactive', 'on')
        .field('user_id', '30')
        .field('user_ip', '192.168.1.100')
        .expect('Content-Type', /json/)
        .expect(201);

      const { body } = response;
      expect(body.status).toEqual(201);
      expect(body.message).toEqual('CREATED');
      expect(body.data.department_id).toEqual('1');
    });
  });
  describe('deleteDepartment', () => {
    it('should return correct information', async () => {
      await Department.create({ departmentId: 1, isdel: 0 });

      const response = await request(app)
        .post('/master/department/restore')
        .field('department_id', '1')
        .field('user_id', '30')
        .field('user_ip', '192.168.1.100')
        .expect('Content-Type', /json/)
        .expect(200);

      const { body } = response;
      expect(body.status).toEqual(200);
      expect(body.message).toEqual('RESTORED');
      expect(body.data.department_id).toEqual(1);
    });
    it('should return bad request when given improper parameter', async () => {
      await Department.create({ departmentId: 1, isdel: 0 });

      const response = await request(app)
        .post('/master/department/restore')
        .field('user_id', '30')
        .field('user_ip', '192.168.1.100')
        .expect('Content-Type', /json/)
        .expect(200);

      const { body } = response;
      expect(body.status).toEqual(200);
      expect(body.message).toEqual('RESTORED');
      expect(body.data.department_id).toEqual(2);
    });
  });
  describe('restoreDepartment', () => {
    it('should return correct information', async () => {
      await Department.create({ departmentId: 1, isdel: 1 });

      const response = await request(app)
        .post('/master/department/delete')
        .field('department_id', '1')
        .field('user_id', '30')
        .field('user_ip', '192.168.1.100')
        .expect('Content-Type', /json/)
        .expect(200);

      const { body } = response;
      expect(body.status).toEqual(200);
      expect(body.message).toEqual('DELETED');
      expect(body.data.department_id).toEqual(1);
    });
    it('should return bad request when given improper parameter', async () => {
      await Department.create({ departmentId: 1, isdel: 0 });

      const response = await request(app)
        .post('/master/department/delete')
        .field('user_id', '30')
        .field('user_ip', '192.168.1.100')
        .expect('Content-Type', /json/)
        .expect(404);

      const { body } = response;
      expect(body.status).toEqual(404);
      expect(body.message).toEqual('Department not found or exist');
    });
  });
});
