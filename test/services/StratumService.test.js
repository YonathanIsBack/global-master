...
describe('StratumService', () => {
  ...
  describe('createStratum', () => {
    ...
    it('should save to designated id if provided', async () => {
      const stratumDto = new StratumDto(createStratumDto(5));
      const stratumService = new StratumService();
      const expectedResult = {
        stratumId: 5,
        stratumCode: 'EVE',
    ...
    it('should update already existing country if country with same row exist', async () => {
      Stratum.create({ stratumCode: 'EVE' });
      Stratum.create({ stratumCode: 'EVE2' });
      const stratumDto = new StratumDto(createStratumDto(2, 'AD', 'Adam'));
      const stratumService = new StratumService();
      const expectedResult = {
        stratumId: 2,
        stratumCode: 'AD',
      ...
      const { data, isUpdated } = await stratumService.createOrUpdate(stratumDto, user, ip);
      const actualResult = await Stratum.findByPk(2);

      expect(data.stratumId).toEqual(expectedResult.stratumId);
      expect(actualResult.stratumId).toEqual(expectedResult.stratumId);
      expect(actualResult.stratumCode).toEqual(expectedResult.stratumCode);
      expect(actualResult.stratumName).toEqual(expectedResult.stratumName);
      expect(actualResult.stratumCode).not.toEqual('EVE');
